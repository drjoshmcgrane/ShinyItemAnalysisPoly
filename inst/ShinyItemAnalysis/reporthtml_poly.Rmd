...
author: |
  ...
  This report was created on `r format(Sys.Date(), "%B %d %Y")` using ShinyItemAnalysis: Test and item analysis, version `r packageVersion("ShinyItemAnalysisPoly")`
...
This report was created by R version `r getRversion()` and its package ShinyItemAnalysis version `r packageVersion("ShinyItemAnalysisPoly")`. ShinyItemAnalysis provides test and item analysis and it is available on [CRAN](https://CRAN.R-project.org/package=ShinyItemAnalysis) and also [online](https://shiny.cs.cas.cz/ShinyItemAnalysis/). 

To cite ShinyItemAnalysis application in publications, please, use:


  Martinková P., \& Hladká A.
  (2023)
...
A Wright map, also called an item-person map, is a graphical tool to display person estimates and item parameters. The person side (left) represents a histogram of estimated abilities. The item side (right) displays item threshold (step) difficulties. For polytomous items, each step is shown with a distinct colour.
")
```

```{r IRT-wright-map-plot, echo = FALSE, eval = show_IRT && has_wrightmap, fig.height = 5, fig.width = 11, fig.align = 'center', dpi = 300, warning = FALSE}
...
Estimates of parameters with standard errors (SE). For Rasch-family models (RSM/PCM), infit and outfit mean-square statistics are also reported. Column names reflect the selected model's parametrisation.
")
```
...
tab <- params$irt_coef
if (!is.null(tab)) {
  # Keep native column names from the poly coef reactive; numeric cols rounded.
  num_cols <- vapply(tab, is.numeric, logical(1))
  tab[num_cols] <- lapply(tab[num_cols], round, 2)
  kable(tab, align = c("l", rep("c", max(ncol(tab) - 1, 1))))
}
```
...
The cumulative-logit ordinal DIF method (difORD) tests for uniform and/or non-uniform DIF on polytomous items by fitting an ordinal logistic regression per item and comparing nested models via likelihood-ratio tests. See Hladka & Martinkova (2020) for details.
")
```

```{r DIF-ord-print, eval = show_DIF && has_ord_dif, echo = FALSE, results = 'asis'}
cat("```\n")
cat(params$DIF_ord_print)
cat("\n```")
```

```{r DIF-ord-plot, eval = show_DIF && has_ord_dif && inherits(params$DIF_ord_plot, "ggplot"), echo = FALSE, fig.align = 'center', fig.width = 7, fig.height = 5, dpi = 300}
params$DIF_ord_plot
```
...
The adjacent-category logit ordinal DIF method (difORD) tests for uniform and/or non-uniform DIF on polytomous ordinal items by fitting an adjacent-category logistic regression per item.
")
```

```{r DIF-adj-print, eval = show_DIF && has_adj_dif, echo = FALSE, results = 'asis'}
cat("```\n")
cat(params$DIF_adj_print)
cat("\n```")
```

```{r DIF-adj-plot, eval = show_DIF && has_adj_dif && inherits(params$DIF_adj_plot, "ggplot"), echo = FALSE, fig.align = 'center', fig.width = 7, fig.height = 5, dpi = 300}
params$DIF_adj_plot
```
...
For nominal polytomous items, multinomial log-linear regression detects DDF (differential distractor functioning) by modelling each non-key category against the key.
")
```

```{r DIF-multinom-print, eval = show_DIF && has_multinom_dif, echo = FALSE, results = 'asis'}
cat("```\n")
cat(params$DIF_multinom_print)
cat("\n```")
```

```{r DIF-multinom-plot, eval = show_DIF && has_multinom_dif && inherits(params$DIF_multinom_plot, "ggplot"), echo = FALSE, fig.align = 'center', fig.width = 7, fig.height = 5, dpi = 300}
