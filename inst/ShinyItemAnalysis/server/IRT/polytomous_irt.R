...
# EAP by default, WLE when the checkbox is ticked.
IRT_poly_fscore_method <- reactive({
  if (isTRUE(input$IRT_poly_use_wle)) "WLE" else "EAP"
})
...
IRT_poly_fscores_reactive <- reactive({
  fscore_with_ses <- IRT_poly_fscores_raw()
...
IRT_poly_reliability_reactive <- reactive({
  emp  <- tryCatch(mirt::empirical_rxx(IRT_poly_fscores_raw()),
                   error = function(e) NA_real_)
  marg <- tryCatch(mirt::marginal_rxx(IRT_poly_model()),
                   error = function(e) NA_real_)
  list(method = IRT_poly_fscore_method(), empirical = emp, marginal = marg)
})

output$IRT_poly_summary_ability_reliability_text <- renderText({
  r <- IRT_poly_reliability_reactive()
...
IRT_poly_wrightmap_plots <- reactive({
  ...
  g1 <- ggplot(data.frame(theta = theta), aes(x = .data$theta)) +
    ...
    theme(
      ...
      # match top space with g2's legend so panels align vertically
      plot.margin = margin(t = 18, r = 5, b = 5, l = 5, unit = "pt")
    )
...
output$IRT_poly_summary_wrightmap <- renderPlotly({
  ...
  subplot(plt_left, plt_right, titleY = TRUE, margin = 0.05) |>
    layout(legend = list(
      ...
      orientation = "h",
      font = list(size = 10),
      title = list(font = list(size = 11), side = "left")
    )) |>
...
IRT_poly_items_expected_reactive <- reactive({
  ...
  if (isTRUE(input$IRT_poly_show_observed)) {
    fs <- as.vector(IRT_poly_fscores_raw()[, 1])
