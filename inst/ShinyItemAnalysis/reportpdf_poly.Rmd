...
params:
  ...
  DIF_adj_print: NA
  DIF_adj_plot: NA
  DIF_multinom_print: NA
  DIF_multinom_plot: NA
  data_type: NA
  sessionInfo: NA
---
...
This report was created by `R` version `r getRversion()` and its package `ShinyItemAnalysis` version `r packageVersion("ShinyItemAnalysisPoly")`. \break ShinyItemAnalysis provides test and item analysis and it is available on [CRAN](https://CRAN.R-project.org/package=ShinyItemAnalysis) and also [online](https://shiny.cs.cas.cz/ShinyItemAnalysis/). 
...
\bibitem{martinkova2023}
  Martinkov\'a P., \& Hladk\'a A. (2023)
  \textit{Computational Aspects of Psychometric Methods. With R.} 
  Chapman and Hall/CRC. \\
  https://doi.org/10.1201/9781003054313

\bibitem{martinkova2018}
  Martinkov\'a P., \& Drabinov\'a A. (2018)
...
# turn HTML sub tag into LaTeX
rownames(table) <- str_replace(rownames(table), "<sub>(.*?)</sub>", "\\\\textsubscript{\\1}")
...
print(
  ...
  booktabs = TRUE, comment = FALSE, include.rownames = FALSE,
  tabular.environment = "longtable", floating = FALSE,
  add.to.row = add.to.row, hline.after = c(-1, nrow(table)),
  sanitize.colnames.function = identity # prevent latex escaping in colnames
)
```
...
```{r corr-plot, fig.align = 'center', echo = FALSE, include = any(nzchar(params$corr_plot)), fig.height = 5}
params$corr_plot
```
...
if (params$isCriterionPresent & params$validity_check) {
  validity_table_intro <- "An association between the total score and the criterion variable can be estimated using Pearson product-moment correlation coefficient \\textit{r}. The null hypothesis being tested states that correlation is exactly 0."

  inp <- params$validity_table

  validity_table_results <- inp$txt |>
    as.character() |>
    str_replace_all("<br>", "\\\\textbf{") |>
    str_replace_all("</br>", "}") |>
    str_replace_all("<em>", "\\\\textit{") |>
    str_replace_all("</em>", "}")
  ...
  validity_table_interpretation <- paste(
    "\\textbf{Interpretation:}\\linebreak",
    ifelse(pval < .05,
      paste("The \\textit{p}-value is less than .05, thus we reject the null hypotheses. The total score and criterion variable are", ifelse(est > 0, "positively", "negatively"), "correlated."),
...
```{r validity-plot, echo = FALSE, results = "hide", fig.height = 4.5, fig.width = 11, message = FALSE}
if (params$isCriterionPresent & params$validity_check) {
...
txt2 <- switch(params$DDplotDiscType,
  ...
  "ULI" = if (any(range1 != 1, range2 != 3, numgroups != 3)) {
    paste0(
      "Discrimination (blue) is described by the difference of the percent correct in the ",
      "<b>", range1, "</b>",
...
```{r cronbachs-alpha, echo = FALSE, results = "asis"}
table <- params$cronbachs_alpha_table
add.to.row$command <- command(table)

print(xtable(table, digits = c(0, 3, 0), align = rep("c", 3)),
...
if (params$DDplotNumGroups == 3 & params$DDplotRange1 == 1 & params$DDplotRange2 == 3) {
  ...
  txt2 <- paste0(
    ", gULI - generalized ULI calculated as the difference between the difficulty recorded in the ",
    "<b>", range1, "</b>",
    ifelse(range1 >= 4, "-th", switch(range1,
      "1" = "-st",
      "2" = "-nd",
  ...
  if (params$type_distractor_plot == "Combinations") {
    for (i in 1:length(params$distractor_plot)) {
      ...
      if (is.null(g2)) {
        ...
        g2 <- g2 + theme(
          legend.position = "inside",
          legend.position.inside = c(0.04, 0.97),
          legend.margin = margin(0, 0, 0, 0, "cm"),
    ...
    for (i in 1:length(params$distractor_plot)) {
      g1 <- params$distractor_plot[[i]] + theme(
        legend.position = "inside",
        legend.position.inside = c(0.14, 0.8),
        legend.margin = margin(0, 0, 0, 0, "cm")
      )
      g2 <- params$multiplot[[i]] + theme(
        legend.position = "inside",
        legend.position.inside = c(0.04, 0.97),
        legend.margin = margin(0, 0, 0, 0, "cm"),
        legend.spacing.x = unit(0.25, "cm")
...
```{r IRT-wright-map-available, echo = FALSE}
has_wrightmap <- !is.null(params$irt_wrightmap)
```

```{r IRT-wright-map-text, echo = FALSE, results = "asis", eval = show_IRT && has_wrightmap}
cat("
## Wright (item-person) map

A Wright map, also called an item-person map, is a graphical tool to display person estimates and item parameters. The person side (left) represents a histogram of estimated abilities. The item side (right) displays item threshold (step) difficulties.
")
```

```{r IRT-wright-map-plot, eval = show_IRT && has_wrightmap, echo = FALSE, fig.height = 5, fig.width = 11, warning = FALSE}
plts <- params$irt_wrightmap
if (is.list(plts) && length(plts) == 2) {
...
IRT_selected_model <- switch(params$irt_model,
  "NRM"  = "Nominal Response Model (NRM)",
  "GRM"  = "Graded Response Model (GRM)",
  "RSM"  = "Rating Scale Model (RSM)",
  "PCM"  = "Partial Credit Model (PCM)",
  "GPCM" = "Generalized Partial Credit Model (GPCM)",
  paste0(params$irt_model, " IRT model")
)
```

```{r IRT-equation-text, echo = FALSE, results = "asis", eval = show_IRT}
txt <- "Model parameters are estimated using a marginal maximum likelihood method. Ability $\\theta$ is assumed to follow a standard normal distribution."
...
table <- params$irt_ability_table
add.to.row$command <- command(table)
...
```{r IRT-ability-scatterplot, echo = FALSE, results = "hide", fig.height = 4.5, fig.width = 11, eval = show_IRT}
...
```{r IRT-parameter-estimates-table, echo = FALSE, eval = show_IRT, results = "asis"}
tab <- params$irt_coef
if (!is.null(tab)) {
  ncol_tab <- ncol(tab)
  num_cols <- vapply(tab, is.numeric, logical(1))
  tab[num_cols] <- lapply(tab[num_cols], round, 2)
  add.to.row$command <- command(tab)
  print(
    xtable(tab,
      digits = 2,
      align = c("l", rep("c", ncol_tab))
    ),
    booktabs = T, comment = F, include.rownames = T, sanitize.text.function = function(x) x,
    tabular.environment = "longtable", floating = F, add.to.row = add.to.row,
...
has_ord_dif <- !is.null(params$DIF_ord_print) && nchar(params$DIF_ord_print) > 0 &&
  !grepl("^DIF \\(cumulative logit\\) could not be fit\\.$", params$DIF_ord_print)
has_adj_dif <- !is.null(params$DIF_adj_print) && nchar(params$DIF_adj_print) > 0 &&
  !grepl("^DIF \\(adjacent category logit\\) could not be fit\\.$", params$DIF_adj_print)
has_multinom_dif <- !is.null(params$DIF_multinom_print) && nchar(params$DIF_multinom_print) > 0 &&
  !grepl("^DIF \\(multinomial\\) could not be fit\\.$", params$DIF_multinom_print)
show_DIF <- any(c(params$histCheck, params$deltaplotCheck, params$MHCheck, params$logregCheck, params$multiCheck, has_ord_dif, has_adj_dif, has_multinom_dif))
check_group <- !params$isGroupPresent
```

```{r DIF-section-pagebreak, eval = show_DIF, echo = FALSE, results = 'asis'}
cat("\\pagebreak")
```

```{r DIF-section, eval = show_DIF, echo = FALSE, results = 'asis'}
txt <- ifelse(check_group,
  "No group vector is present. DIF and DDF analyses could have not been generated!",
  ""
)
...
DIF analysis may come to a different conclusion than test of group differences in total scores. Two groups may have the same distribution of total scores, yet, some items may function differently for the two groups. Also, one of the groups may have a signifficantly lower total score, yet, it may happen that there is no DIF item [(see Martinkova et al., 2017)](https://doi.org/10.1187/cbe.16-10-0307).
\\vspace{-1.5em}
...
table <- params$DIF_total_table
add.to.row$command <- command(table)

colnames(table) <- str_replace(colnames(table), "<sub>(.*?)</sub>", "\\\\textsubscript{\\1}")

print(
  xtable(table,
       digits = c(rep(0, 6), rep(2, 5)),
       align = c("l", rep("c", 10))
       ),
...
### Histograms of total scores by group \n \\vspace{-0.5em}"))
```

```{r DIF-section-histograms-plot, echo = FALSE, fig.height = 4.3, fig.width = 11, eval = eval_histograms}
...
```{r DIF-section-histograms-ttest, echo = FALSE, results = "asis", eval = eval_histograms}
table <- params$DIF_total_ttest
add.to.row$command <- command(table)

print(xtable(table, align = rep("c", 5)),
  include.rownames = F,
  booktabs = T, comment = F,
  tabular.environment = "longtable", floating = F,
  add.to.row = add.to.row, hline.after = c(-1, nrow(table))
...
```{r DIF-section-histograms-part4, eval = eval_histograms, echo = FALSE, results = 'asis'}
cat("\\pagebreak")
```
...
```{r DIF-section-deltaplot-table, echo = FALSE, results = "asis", eval = eval_deltaplot}
tab <- cbind(res_deltaplot$Props, res_deltaplot$Deltas, res_deltaplot$Dist[, ncol(res_deltaplot$Dist)])
...
txt <- ifelse(ncol(tab) < 26, "\\pagebreak", "")
cat(paste(
  txt,
  "
...
```{r DIF-section-deltaplot-plot, echo = FALSE, fig.height = 4.5, fig.width = 11, eval = eval_deltaplot}
...
```{r DIF-section-deltaplot-part3, eval = eval_deltaplot, echo = FALSE, results = 'asis'}
cat("\\pagebreak")
```
...
```{r DIF-section-MH-table, echo = FALSE, results = "asis", eval = eval_MH}
...
tab <- cbind(
  sprintf("%.3f", res_MH$MH), pval,
  sprintf("%.3f", res_MH$alphaMH), deltaMH
)
rownames(tab) <- params$itemNames

if (dim(tab)[2] == 5) {
  colnames(tab) <- c("Stat.", "P-value", "Adj. p-value", "$\\alpha_{\\text{MH}}$", "$\\delta_{\\text{MH}}$")
  print(
    xtable(tab,
      digits = c(0, 3, 3, 0, 0, 0),
      align = c("l", "r", "r", "l", "r", "r")
    ),
    booktabs = T, comment = F, tabular.environment = "longtable", floating = F,
    sanitize.text.function = function(x) {
      x
    }, add.to.row = add.to.row, hline.after = c(-1, nrow(tab))
  )
} else {
  colnames(tab) <- c("Stat.", "P-value", "$\\alpha_{\\text{MH}}$", "$\\delta_{\\text{MH}}$")
  print(
    xtable(tab,
      digits = c(0, 3, 0, 0, 0),
      align = c("l", "r", "l", "r", "r")
...
cat("Signif. codes: 0 '\\mbox{*}\\mbox{*}\\mbox{*}' 0.001 '\\mbox{*}\\mbox{*}' 0.01 '\\mbox{*}' 0.05 '.' 0.1 '&nbsp;' 1
...
Effect size codes: 0 'A' 1.0 'B' 1.5 'C' (for absolute values of $\\delta_{\\text{MH}}$)
")
```

```{r DIF-section-MH-part3, eval = eval_MH, echo = FALSE, results = 'asis'}
cat("\\pagebreak")
```
...
txt2 <- paste(
  ifelse(res_logistic$purification,
    "Item purification was used.",
    "Item purification was not applied."
  ),
  ifelse(is.null(res_logistic$p.adjust.method),
    "No p-value adjustment for multiple comparisons was used.",
    paste(
      switch(res_logistic$p.adjust.method,
        "holm" = "Holm's",
        "hochberg" = "Hochberg's",
        "hommel" = "Hommel's",
        "bonferroni" = "Bonferroni",
...
```{r DIF-section-logistic-table, echo = FALSE, results = "asis", eval = eval_logistic}
if (res_logistic$p.adjust.method == "none") {
  ...
  pval <- cbind(
    sprintf("%.3f", res_logistic$p.value),
...
tab <- cbind(
  sprintf("%.3f", res_logistic$Logistik), pval,
...
```{r DIF-section-logistic-plot, echo = FALSE, fig.height = 4.1, fig.width = 11, eval = eval_logistic_plot}
...
```{r DIF-section-logistic-part4, eval = eval_logistic, echo = FALSE, results = 'asis'}
cat("\\pagebreak")
```
...
txt3 <- ifelse(is.null(res_DDF$p.adjust.method),
  "No p-value adjustment for multiple comparisons was used.",
  paste(
    switch(res_DDF$p.adjust.method,
      "holm" = "Holm's",
      "hochberg" = "Hochberg's",
      "hommel" = "Hommel's",
      "bonferroni" = "Bonferroni",
...
```{r DIF-section-DDF-table, echo = FALSE, results = "asis", eval = eval_DDF}
...
cat("
Signif. codes: 0 '\\mbox{*}\\mbox{*}\\mbox{*}' 0.001 '\\mbox{*}\\mbox{*}' 0.01 '\\mbox{*}' 0.05 '.' 0.1 '&nbsp;' 1
")
```
...
```{r DIF-section-DDF-plot, echo = FALSE, fig.height = 4.1, fig.width = 11, eval = eval_DIF_plot}
...
```{r DIF-ord-section, eval = show_DIF && has_ord_dif, echo = FALSE, results = 'asis'}
cat("
\\pagebreak

## DIF detection using cumulative logit (difORD)

The cumulative-logit ordinal DIF method (difORD) tests for uniform and/or non-uniform DIF on polytomous items. See Hladka & Martinkova (2020) for details.

\\begin{verbatim}
", params$DIF_ord_print, "
\\end{verbatim}
")
```

```{r DIF-ord-plot, eval = show_DIF && has_ord_dif && inherits(params$DIF_ord_plot, "ggplot"), echo = FALSE, fig.width = 8, fig.height = 5}
params$DIF_ord_plot
```

```{r DIF-adj-section, eval = show_DIF && has_adj_dif, echo = FALSE, results = 'asis'}
cat("
\\pagebreak

## DIF detection using adjacent category logit (difORD)

The adjacent-category logit ordinal DIF method (difORD) tests for uniform and/or non-uniform DIF on polytomous ordinal items.

\\begin{verbatim}
", params$DIF_adj_print, "
\\end{verbatim}
")
```

```{r DIF-adj-plot, eval = show_DIF && has_adj_dif && inherits(params$DIF_adj_plot, "ggplot"), echo = FALSE, fig.width = 8, fig.height = 5}
params$DIF_adj_plot
```

```{r DIF-multinom-section, eval = show_DIF && has_multinom_dif, echo = FALSE, results = 'asis'}
cat("
\\pagebreak

## DIF detection using multinomial regression (difNLR)

For nominal polytomous items, multinomial log-linear regression detects DDF by modelling each non-key category against the key.

\\begin{verbatim}
", params$DIF_multinom_print, "
\\end{verbatim}
")
```

```{r DIF-multinom-plot, eval = show_DIF && has_multinom_dif && inherits(params$DIF_multinom_plot, "ggplot"), echo = FALSE, fig.width = 8, fig.height = 5}
params$DIF_multinom_plot
```
